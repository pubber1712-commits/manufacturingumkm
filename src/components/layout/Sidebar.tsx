...
export default function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-bg-side h-full flex flex-col border-r border-border-dim shrink-0 hidden md:flex">
        ...
        <nav className="space-y-1">
          {filteredMenu.map((item) => (
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 px-4 py-3 text-[10px] uppercase tracking-[0.2em] font-medium transition-all group",
                activeTab === item.id 
